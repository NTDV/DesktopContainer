...
namespace Microsoft.WindowsAPICodePack.Shell
{
    ...
    public class ShellSearchFolder : ShellSearchCollection
    {
        ...
        /// <param name="searchScopePath">
        ///     List of folders/paths to perform the search on. These locations need to be indexed by the
        ///     system.
        /// </param>
        ...
        public IEnumerable<string> SearchScopePaths
        {
            get
            {
                foreach (var scopePath in searchScopePaths) yield return scopePath;
            }
            private set
            {
                ...
                foreach (var path in searchScopePaths)
                {
                    ...
                    var hr = ShellNativeMethods.SHCreateItemFromParsingName(path, IntPtr.Zero, ref shellItemGuid,
                        out scopeShellItem);
        ...
        internal override IShellItem NativeShellItem
        {
            get
            {
                ...
                var hr = NativeSearchFolderItemFactory.GetShellItem(ref guid, out shellItem);
        ...
        public void SetStacks(params string[] canonicalNames)
        {
            ...
            foreach (var prop in canonicalNames)
            {
                ...
                PropertyKey propKey;
                var result = PropertySystemNativeMethods.PSGetPropertyKeyFromName(prop, out propKey);

                if (!CoreErrorHelper.Succeeded(result))
                    throw new ArgumentException("ShellInvalidCanonicalName", nameof(canonicalNames),
                        Marshal.GetExceptionForHR(result));

                propertyKeyList.Add(propKey);
            }

            if (propertyKeyList.Count > 0) SetStacks(propertyKeyList.ToArray());
        }
        ...
        public void SetVisibleColumns(PropertyKey[] value)
        {
            ...
            if (!CoreErrorHelper.Succeeded(hr))
                throw new ShellException("ShellSearchFolderUnableToSetVisibleColumns",
        ...
        public void SortColumns(SortColumn[] value)
        {
            ...
            if (!CoreErrorHelper.Succeeded(hr))
                throw new ShellException("ShellSearchFolderUnableToSetSortColumns",
                    Marshal.GetExceptionForHR((int) hr));
        }
