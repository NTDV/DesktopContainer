    ...
    internal static class ShellObjectFactory
    {
        ...
        internal static ShellObject Create(IShellItem nativeShellItem)
        {
            ...
            if (isFolder)
            {
                ...
                if (itemType == ".search-ms") return new ShellSavedSearchCollection(nativeShellItem2);
                ...
                lock (padlock)
                {
                    ...
                    ThreadPool.QueueUserWorkItem(obj =>
                    {
                        lock (padlock)
                        {
                            ...
                            if (nativeFolder != null) nativeFolder.GetFolderDefinition(out definition);
        ...
        {
            if (string.IsNullOrEmpty(parsingName)) throw new ArgumentNullException(nameof(parsingName));

            // Create a native shellitem from our path
            IShellItem2 nativeShellItem;
            var guid = new Guid(ShellIIDGuid.IShellItem2);
            var retCode =
                ShellNativeMethods.SHCreateItemFromParsingName(parsingName, IntPtr.Zero, ref guid, out nativeShellItem);

            if (!CoreErrorHelper.Succeeded(retCode))
                throw new ShellException("ShellObjectFactoryUnableToCreateItem", Marshal.GetExceptionForHR(retCode));
