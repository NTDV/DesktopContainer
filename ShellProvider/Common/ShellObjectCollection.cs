...
namespace Microsoft.WindowsAPICodePack.Shell
{
    ...
    public class ShellObjectCollection : IEnumerable, IDisposable, IList<ShellObject>
    {
        ...
        {
            foreach (var obj in content) yield return obj;
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (isDisposed == false)
            {
                if (disposing)
                {
                    foreach (var shellObject in content) shellObject.Dispose();
        ...
        {
            if (content.Count == 0) throw new InvalidOperationException("ShellObjectCollectionEmptyCollection");
            ...
            try
            {
                ...
                foreach (var offset in offsets) bwriter.Write(offset);
        ...
        {
            if (IsReadOnly) throw new InvalidOperationException("ShellObjectCollectionInsertReadOnly");
        ...
        {
            if (IsReadOnly) throw new InvalidOperationException("ShellObjectCollectionRemoveReadOnly");
        ...
        public void CopyTo(ShellObject[] array, int arrayIndex)
        {
            ...
            if (array.Length < arrayIndex + content.Count)
                throw new ArgumentException("ShellObjectCollectionArrayTooSmall", nameof(array));

            for (var index = 0; index < content.Count; index++) array[index + arrayIndex] = content[index];
        }
